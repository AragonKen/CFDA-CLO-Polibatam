<template>
  <VCard
    title="Percentage of Students within Each Category"
    subtitle="Percentage of Students within Each Category of Assessment Type"
  >
    <VCardText>
      <VTable
...
onMounted(async () => {
  await store.dispatch('assessment/GetPercentageStudentPerCategory', props.assessmentId).then(response => {
    data.value = response
