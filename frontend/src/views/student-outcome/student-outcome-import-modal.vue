<template>
  ...
  >
    ...
    <VCard
      title="Import Student Outcome"
      subtitle="Import Student Outcome from excel file"
    >
      <VDivider class="mt-3" />
...
const onDownloadTemplate = () => {
  ...
  let settings = {
    fileName: `Import Format Student Outcome`, 
    extraLength: 3,
    writeMode: "writeFile",
  }
...
const onImport = async e => {
  ...
  reader.onload = function (e) {
    ...
    for (const iterator of json) {
      formatted_json.push({
        code: iterator["Code"],
        description: iterator["Student Outcome"],
      })
    }

    commit("studentOutcome/SET_FORM_BULK", formatted_json)
...
const onSubmit = async () => { 
  await store.dispatch('studentOutcome/BulkCreate', {
