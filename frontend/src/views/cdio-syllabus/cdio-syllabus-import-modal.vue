<template>
  ...
  >
    ...
    <VCard
      title="Import CDIO Syllabus"
      subtitle="Import CDIO Syllabus from excel file"
    >
      <VDivider class="mt-3" />
...
const onDownloadTemplate = () => {
  window.open(
    `/Import Format CDIO Syllabus.xlsx`,
    "_blank",
  )
}
...
const onImport = async e => {
  ...
  reader.onload = function (e) {
    ...
    for (const iterator of json) {
      formatted_json.push({
        description: iterator["CDIO SYLLABUS"],
      })
    }

    commit("CDIOSyllabus/SET_FORM_BULK", formatted_json)
...
const onSubmit = async () => { 
  await store.dispatch('CDIOSyllabus/BulkCreate', {
